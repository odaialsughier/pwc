...
class ApiResponseBehavior extends Behavior
{
    ...
    {
        //throw new \yii\web\BadRequestHttpException(Yii::t('app',$message), $status_code);
        $this->setHeader($header_status);
        $response = array(
            'code' => $code,
            'name' => $name,
            'message' =>    $message,//is_array($message) ? $message : Yii::t('app', $message),
        ...
        if ($message instanceof Model) {
            ...
            $output = $serializer->serialize($message);
            $response['message'] = $output;
        }
        
        if(!empty($errorMask)) {
            if(is_array($errorMask)){
               $response['errorMask'] = !empty($errorMask['errorMask']) ? Yii::t('app',$errorMask['errorMask']) : '';
               $response['errorGroup'] = !empty($errorMask['errorGroup']) ? $errorMask['errorGroup'] : '';
            }else{
